...
(*                                                +#+#+#+#+#+   +#+           *)
(*   Created: 2015/06/19 17:28:05 by ngoguey           #+#    #+#             *)
(*   Updated: 2015/06/19 18:05:35 by ngoguey          ###   ########.fr       *)
(*                                                                            *)
(* ************************************************************************** *)
...
let newDeck () =
  Random.self_init ();
  let lassoc = List.map (fun c -> (c, Random.int 2_000_000)) Card.all in
  let lassoc' = List.stable_sort (fun (_, i) (_, i') -> i - i') lassoc in
  List.map (fun (c, _) -> c) lassoc'

let toStringList l =
  List.map (fun c -> Card.toString c) l

let toStringListVerbose l =
  List.map (fun c -> Card.toStringVerbose c) l

let drawCard = function
  | []						-> raise (Failure "no cards")
  | hd::tl					-> (hd, tl)
